/**
 * 全局工具函数
 * 从 src/js/app/global_ability.js 迁移
 */
 ...
 */
export function sortByLevel(array: Array<{ 对象: any; 优先级: string | number }>): any[] {
    if (array.length === 1) {
        return [array[0].对象]
    }

    const priorityQueues: Record<string | number, any[]> = {}
    // 遍历影响数组，获取以优先级为key的字典，将相同优先级的object放在一起
    for (let i = 0; i < array.length; i++) {
        const object = array[i].对象
        const priority = array[i].优先级
    ...
    const priorities = Object.keys(priorityQueues).sort((a, b) => {
 ...
 */
export function expandJsObject(target: any, more: any): any {
    // 遍历more里的键
 ...
 * 判断一个字符串是否是可计算的
 */
export function isCalculableString(string: string): boolean {
    try {
        // 使用eval函数尝试执行字符串表达式
        eval(string)
 ...
 */
export function countValue(base: any, string: any): any {
 ...
 */
export function negativeValue(string: any): any {
    ...
    // 提取数值部分并根据符号进行转换
    const value = parseFloat(string) // 提取数值部分，例如将 "+12" 提取为 12
    let result: string
 ...
 *
 * 注意：此函数依赖jQuery，在Vue3中应该使用原生DOM API重写
 * 暂时保留以供参考
 */
export function getOffsetBeside(
    target: HTMLElement,
    self?: HTMLElement,
    vertical: 'top' | 'bottom' | 'center' | 'same_top' | 'same_bottom' = 'same_top',
    horizontal: 'left' | 'right' | 'center' | 'same_left' | 'same_right' = 'right'
    ...
    let selfSize: { width: number; height: number } | undefined
    if (self) {
        selfSize = {
    ...
    switch (vertical) {
        case 'top':
            top = targetPosition.top - (selfSize?.height || 0)
            break
        case 'bottom':
            top = targetPosition.top + targetSize.height
            break
        case 'center':
            top = targetPosition.top + targetSize.height / 2 - (selfSize?.height || 0) / 2
            break
        case 'same_top':
            top = targetPosition.top
            break
        case 'same_bottom':
            top = targetPosition.top - ((selfSize?.height || 0) - targetSize.height)
            break
        default:
    ...
    switch (horizontal) {
        case 'left':
            left = targetPosition.left - (selfSize?.width || 0)
            break
        case 'right':
            left = targetPosition.left + targetSize.width
            break
        case 'center':
            left = targetPosition.left + targetSize.width / 2 - (selfSize?.width || 0) / 2
            break
        case 'same_left':
            left = targetPosition.left
            break
        case 'same_right':
            left = targetPosition.left - ((selfSize?.width || 0) - targetSize.width)
            break
        default:
 ...
 * 报错
 */
export function newError(error_code: string, error_text_parts: any[]): never {
    // 拼接所有文本部分
