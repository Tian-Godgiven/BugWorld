 ...
 * 在模块加载时初始化
 */
let Event_tendency_lib: Record<number, string[]> = {}

/**
 * 初始化事件倾向库
 */
function initEventTendencyLib(): void {
    for (let key in Event_lib) {
        const event_json = Event_lib[key as keyof typeof Event_lib]
 ...
 * 创建一个事件对象
 */
export function createEvent(key: string, source: any): Event | false {
    const event = new Event()
    // 通过event_key获取到对应的事件数据
    const json = Event_lib[key as keyof typeof Event_lib]
    if (!json) {
        console.log("不存在指定的event_key：" + key)
        return false
    }
    const func = _.cloneDeep((Event_func_lib as any)[key])
    // 如果事件对象的[功能→强度]为true，则其拥有一个强度=0的初始值
    const more_state: Record<string, any> = {}
 ...
 * 随机发生一个事件
 */
export function happenEvent(bugNest: any, source: any): boolean {
    // 判定是否会发生事件
    ...
    if (random_num > 事件信息.概率边界) {
        ...
        while (events && events.length > 0 && !tmp) {
            ...
            if (event && startEvent(event, bugNest, source, 强度)) {
 ...
 * @returns 是否成功停止
 */
export function stopEvent(event: Event, bugNest: any): boolean {
    // 检查事件是否正在进行中
    if (!hiddenValue(event, '进行中')) {
        console.log('该事件未在进行中，无法停止')
        return false
    }

    // 触发事件的"结束"行为（如果存在）
    runObjectMovement(event, '结束', bugNest)

    // 从虫巢[进行中→事件]中移除
    const 进行中事件 = hiddenValue(bugNest, ['进行中', '事件'])
    const index = 进行中事件.indexOf(event)
    if (index !== -1) {
        进行中事件.splice(index, 1)
    }

    // 修改事件为未进行
    setHidden(event, '进行中', false)

    // 清除事件的作用范围
    changeState(event, '范围', null)

    // 如果事件有强度影响，移除该影响
    if (event.功能.强度 !== false) {
        // 移除强度相关的Impact
        const 强度State = event.属性.强度
        if (强度State && 强度State.影响) {
            // 清空影响数组
            强度State.影响 = []
        }
    }
