...
import Event_lib from '../../library/data/Event_lib.json'
import * as Event_func_lib from '../../library/functions/Event_func_lib'
...
let Event_tendency_lib: Record<string, string[]> = {}

// 初始化事件倾向lib
function initEventTendencyLib(): void {
  for (const key in Event_lib) {
    const event_json = (Event_lib as any)[key]
...
// 创建一个事件对象
export function createEvent(key: string, source: any): any {
  const event = new Event()
  // 通过event_key获取到对应的事件数据
  const json = (Event_lib as any)[key]
  if (!json) {
...
export function happenEvent(bugNest: any, source: any): boolean {
  ...
  const random_num = _.random(初始事件信息.发生事件min, 初始事件信息.发生事件max + 繁荣)
  ...
  if (random_num > 事件信息.概率边界) {
    ...
    while (events && events.length > 0 && !tmp) {
      // 随机选择一个事件
      const random_index = _.random(0, events.length - 1)
...
// 使得一个事件对象停止
export function stopEvent(event: any, bugNest: any): void {
  // 待实现
}
