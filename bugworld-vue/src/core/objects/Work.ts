...
import Work_lib from '../../library/data/Work_lib.json'
import * as Work_func_lib from '../../library/functions/Work_func_lib'
...
// 创建一个工作对象
export function createWork(work_key: string, source: any): any {
  const json = (Work_lib as any)[work_key]
  if (!json) {
    console.log('不存在指定的work_key：' + work_key)
    return false
  }
  const func = _.cloneDeep((Work_func_lib as any)[work_key])
...
// 通过数据来创建工作对象
export function createWorkWithData(key: string, source: any, json: any, func: any): any {
  const work = new Work()
  // 初始化
...
// 令虫巢解锁一个指定的工作对象
export function unlockWorkToBugNest(work: any | string, bugNest: any, source?: any): void {
...
export function deleteWorkFrom(target: any, work_source: any, work?: any): boolean {
...
// 令某个工作开始进行，返回相应的工作对象
export function startWork(bugNest: any, work: any | string): any {
...
export function stopWork(work: any, type?: string): boolean {
  if (type === '中断') {
    runObjectMovement(work, '中断')
  }

  // 所有被该工作占用的对象分别辞职
  for (const object of work.占有) {
  ...
  // 从工作Tile中删除[data→work]为该工作的workDiv（由UI层处理）
...
export function joinWork(bug: any, num: number, work: any): void {
  // 【虫群对象】符合【工作】的需求行为的要求
  if (runObjectMovement(work, '需求', [bug, num])) {
    ...
    if (num <= free_num) {
      ...
      // 计算并提升工作的总效率
      const 效率 = countWorkEfficiency(work, bug, num)
      const 总效率 = countValue(hiddenValue(work, '总效率'), 效率)
      setHidden(work, '总效率', 总效率)
      // 通知工作对象更新
      notifyObjectUpdate(work)
      notifyObjectUpdate(stateValue(work, '所属'))
    }
  }
}

// 令指定虫群对象的指定个数退出指定的工作
export function resignWork(bug: any, num: number | 'all', work: any): void {
  // 对象要具备这个工作
...
// 计算对象参加事务后提供的效率并返回
export function countWorkEfficiency(work: any, object: any, num: number, type?: string): any {
  ...
  if (效率 != null) {
    ...
    if (/[+-]/.test(效率)) {
      if (value !== 0) {
        ...
        value = simbol + Math.abs(value as number)
      }
    }
