...
import Chara_lib from '../../library/data/Characteristic_lib.json'
import * as Chara_func_lib from '../../library/functions/Characteristic_func_lib'
...
// 创建一个特性对象
export function createCharacteristic(chara_key: string, source: any): any {
  const chara = new Characteristic()
  const chara_state = (Chara_lib as any)[chara_key]
...
export function bindObjectCharacteristic(object: any, source?: any): void {
  ...
  for (const chara_key of 特性) {
    // 创建特性对象
    const chara = createCharacteristic(chara_key, source)
    // 使得对象获得特性对象
...
export function getCharacteristic(object: any, chara: any): void {
  // 修改【特性对象】的"所属"
...
export function loseCharacteristic(object: any, chara: any, source: any): void {
  // 触发其"失去"行为
...
// 触发一个特性的指定函数
export function runCharaFunction(chara: any, func_key: string): void {
  // 触发指定的函数
