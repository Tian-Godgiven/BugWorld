...
import Facility_lib from '../../library/data/Facility_lib.json'
import Facility_Work_lib from '../../library/data/Facility_Work_lib.json'
import * as Facility_func_lib from '../../library/functions/Facility_func_lib'
import * as Facility_Work_func_lib from '../../library/functions/Facility_Work_func_lib'
...
export function createFacility(
  ...
  // 初始化
  const json = (Facility_lib as any)[facility_key]
  if (!json) {
    console.log('指定的facility_key不存在')
    return false
  }
  const more_states: any = {}
  if (num) {
    more_states['数量'] = num
  }
...
export function createFacilityWork(
  ...
  if (facility_key instanceof Facility) {
    facility_key = (facility_key as any).key
  }
  // 获取设施对应的工作
  let facilityWork = (Facility_Work_lib as any)[facility_key]
  let facilityWorkFunc = (Facility_Work_func_lib as any)[facility_key]
  if (!facilityWork) {
  ...
  // 创建work对象
  const work = createWorkWithData(
    _.isArray(work_key) ? work_key[work_key.length - 1] : work_key,
    work_source,
    json,
    func
  )
...
export function unlockFacilityToBugNest(
  ...
  // 判断该设施对象是否存在
  if ((Facility_lib as any)[facility_key]) {
    // 创建对应的设施对象的"建造"工作
...
// 令虫巢获得指定设施
export function facilityJoinToBugNest(facility: any, bugNest: any): boolean {
  // 判断该虫巢是否满足添加设施对象的需求
  if (runObjectMovement(facility, '加入需求', bugNest)) {
    ...
    // 如果还没有同key的设施，则创建对应的数组
      ...
      // @ts-ignore - TypeScript无法正确推断动态属性类型
