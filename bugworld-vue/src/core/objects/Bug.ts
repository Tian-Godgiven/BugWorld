...
import Bug_lib from '../../library/data/Bug_lib.json'
import * as Bug_func_lib from '../../library/functions/Bug_func_lib'
import { stateValue, changeState } from '../state/State'
...
// 创建虫群对象
export function createBug(bug_key: string, num: number, source: any, states?: any): any {
  const bug = new Bug()
  // 从lib中获取对象的属性数据
  const bug_state = (Bug_lib as any)[bug_key]
  // 初始化一些属性和参数
...
// 令虫群对象被一个来源所占有
export function occupyBug(bug: any, need_num: number | '剩余', source: any): boolean {
  ...
  // 通知对象更新
  notifyObjectUpdate(bug)
  notifyObjectUpdate(source)
  return true
}

// 解除虫群对象的占用
export function unoccupyBug(bug: any, bug_num: number | 'all', source: any): boolean {
  let occupy: any = null
  let occupy_index: number = -1
  const 被占有 = hiddenValue(bug, '被占有')
  for (const i of 被占有) {
    if (source === i.占有来源) {
      occupy = i
...
// 获得一个指定占有对应的占有信息
export function getOccupyBug(bug: any, source: any): number {
  const 被占有 = hiddenValue(bug, '被占有')
  for (const i of 被占有) {
    if (source === i.占有来源) {
      return i.占有数量
...
// 获得一个虫群对象当中未被占有的数量
export function getFreeBug(bug: any): number {
...
export function bugJoinTo(bug: any, target: any, source?: any): boolean {
  // 判断目标是否满足bug对象的加入需求
  if (runObjectMovement(bug, '加入需求', target)) {
    ...
    const bug_key = (bug as any).key
    const bugGroup: any = stateValue(target, '虫群')
...
// 返回对象是否参与了某个事务
export function haveOccupy(object: any, source: any): boolean {
  // 遍历对象的[被占有]属性
