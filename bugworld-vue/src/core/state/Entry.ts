...
// 使得对象获得词条
export function getEntry(object: any, entry: string): void {
...
export function haveEntry(object: any, ...entrys: any[]): boolean {
  ...
  for (const entry of entrys) {
    ...
    if (_.isArray(entry)) {
      bool = _.every(词条, (element: any) => _.includes(entry, element))
    }
    // 否则判断对象词条当中是否具备这个词条
    else {
...
// 使得对象失去指定的词条
export function loseEntry(object: any, entry: string): void {
