...
export interface MovementEffect {
  ...
  优先级: number | string
  来源: any
  效果: ((object: any, ...args: any[]) => any) | null
}
...
export function initMovement(object: any, func_lib: any, move_belong: string[] = []): void {
  // 初始化过程中，对象的行为来源于对象自身
  const move_source = object
  // 遍历func_lib，将其中的行为对象添加到对象中
  for (const move_key in func_lib) {
...
export function addMovement(
  ...
  if (_.has(move_data, '之前') || _.has(move_data, '当时') || _.has(move_data, '之后')) {
    for (const move_time in move_data) {
      // 然后再用其中的内容生成effect对象
  ...
  // 否则，如果其是一个字典，则认为其是一个行为对象字典
    ...
    const new_belong = [...move_belong, move_key]
    // 遍历字典中的内容，递归调用本函数
    for (const new_move_key in move_data as any) {
      const new_move_data = (move_data as any)[new_move_key]
      addMovement(object, new_move_key, new_move_data, move_source, new_belong)
    }
  }
  // 如果都不是，就报错
  else {
...
export function addMovementEffect(
  ...
  // 找到对应的行为对象
  let 行为对象: any = getMovementInObject(object, move_key)
  ...
  if (行为对象 === false) {
    ...
    if (type === 'new') {
      ...
      if (_.isArray(move_key)) {
        ...
        final_key = move_belong.pop()!
      } else {
        final_key = move_key
      }
      const move_data: any = {}
      move_data[move_time] = move_effect
      addMovement(object, final_key, move_data, move_source, move_belong)
      return true
    }
    // 否则报错！不应该允许这种情况存在！
    else {
...
export function deleteMovementEffect(
  ...
  if (行为对象) {
    ...
    // 否则遍历所有时间标识
      ...
      for (const mt in 行为对象) {
        deleteMovementEffect_inner(行为对象[mt], source)
      }
    }
  }
  // 如果这个行为尚未存在，则返回false
  else {
    console.log(`指定的行为在对象中尚不存在！`)
    return
  }
...
export function getMovementInObject(object: any, move_key: string | string[]): any {
...
// 修改一个对象的指定函数（保留原有接口，暂未实现）
export function changeFunction(object: any, func_name: string, func: Function): void {
